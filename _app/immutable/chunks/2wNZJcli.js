...
`,l={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},u={random:1,"2x2":2,"4x4":3,"8x8":4};function d(e){if(Array.isArray(e))return e.length===4?e:e.length===3?[...e,1]:_;if(typeof e!=`string`)return _;let t,n,r,i=1;if(e.startsWith(`#`))[t,n,r,i]=f(e);else if(e.startsWith(`rgb`))[t,n,r,i]=p(e);else if(e.startsWith(`hsl`))[t,n,r,i]=h(m(e));else return console.error(`Unsupported color format`,e),_;return[g(t,0,1),g(n,0,1),g(r,0,1),g(i,0,1)]}function f(e){return e=e.replace(/^#/,``),e.length===3&&(e=e.split(``).map(e=>e+e).join(``)),e.length===6&&(e+=`ff`),[parseInt(e.slice(0,2),16)/255,parseInt(e.slice(2,4),16)/255,parseInt(e.slice(4,6),16)/255,parseInt(e.slice(6,8),16)/255]}function p(e){let t=e.match(/^rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([0-9.]+))?\s*\)$/i);return t?[parseInt(t[1]??`0`)/255,parseInt(t[2]??`0`)/255,parseInt(t[3]??`0`)/255,t[4]===void 0?1:parseFloat(t[4])]:[0,0,0,1]}function m(e){let t=e.match(/^hsla?\s*\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(?:,\s*([0-9.]+))?\s*\)$/i);return t?[parseInt(t[1]??`0`),parseInt(t[2]??`0`),parseInt(t[3]??`0`),t[4]===void 0?1:parseFloat(t[4])]:[0,0,0,1]}function h(e){let[t,n,r,i]=e,a=t/360,o=n/100,s=r/100,c,l,u;if(n===0)c=l=u=s;else{let e=(e,t,n)=>(n<0&&(n+=1),n>1&&--n,n<1/6?e+(t-e)*6*n:n<1/2?t:n<2/3?e+(t-e)*(2/3-n)*6:e),t=s<.5?s*(1+o):s+o-s*o,n=2*s-t;c=e(n,t,a+1/3),l=e(n,t,a),u=e(n,t,a-1/3)}return[c,l,u,i]}var g=(e,t,n)=>Math.min(Math.max(e,t),n),_=[0,0,0,1];export{l as DitheringShapes,u as DitheringTypes,n as ShaderMount,c as ditheringFragmentShader,d as getShaderColorFromString};